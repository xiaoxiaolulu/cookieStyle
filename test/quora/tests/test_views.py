...
class TestQuestionListView(BaseQATest):
    ...
    def test_context_data(self):
        ...
        self.assertTrue(all(a == b for a, b in zip(response.context_data['question_list'], Question.objects.all())))
...
class TestCreateAnswerView(BaseQATest):
    ...
    def get_post(self):
        ...
        response = self.post(views.AnswerCreateView, request=request)
        assert response.status_code == 302
        assert response.url == f'/quora/question-detail/{self.question_one.id}'
...
class TestQAVote(BaseQATest):
    ...
    def test_question_upvote(self):
        """赞同问题"""
        self.request.POST['questionId'] = self.question_one.id
    ...
    def test_question_downvote(self):
        """反对问题"""
        self.request.POST['questionId'] = self.question_two.id
...
#
# from mydjango.qa.models import Question, Answer
#
#
